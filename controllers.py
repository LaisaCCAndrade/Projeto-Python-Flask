...
def login(email, password):
    try:
        user = User.query.filter(or_(
            User.email == email, User.username == email)).first()
        if user:
...
def reportByState(state, disease):
    patients = db.session.query(db.func.count(
        Patient.estadoSaude), Patient.last_state, Patient.state).group_by(
        Patient.last_state).group_by(Patient.state)

    if state:
        patients = patients.filter(Patient.state == state)
    if disease:
        patients = patients.filter(Patient.diseases.any(
            Disease.id.in_(disease)))
