...
disease_patient = db.Table('disease_patient', db.Column(
    'disease_id', db.Integer, db.ForeignKey('disease.id')),
...
class Patient(db.Model):
    ...
    diseaseState = db.Column(
        db.Integer, db.ForeignKey(DiseaseState.id), nullable=False)
    estado = relationship(State)
    last_state = db.Column(
        db.Date, onupdate=db.func.current_timestamp(), nullable=True)
    estadoSaude = relationship(DiseaseState)
    diseases = db.relationship(
        'Disease', secondary=disease_patient, backref=db.backref(
            'patients', lazy=True))
